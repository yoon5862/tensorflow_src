...
//hack here
TfLiteStatus opsToReplace(TfLiteContext* context, TfLiteDelegate* delegate, TfLiteIntArray *&ops_to_replace);
TfLiteStatus CustomDelegatePrepare(TfLiteContext* context, TfLiteDelegate* delegate, TfLiteIntArray* ops_to_replace);
...
class Delegate {
 ...
 private:
  ...
  TfLiteDelegate delegate_ = {
      ...
      opsToReplace,                   // .opt_node
      CustomDelegatePrepare,          // .CustomPrepare
  };
...
TfLiteStatus DelegatePrepare(TfLiteContext* context, TfLiteDelegate* delegate) {
  ...
  // for(int i = 0; i < ops_to_replace_size; i++){
  //   TfLiteIntArray *ops_to_replace_part = ConvertArrayToTfLiteIntArray(1, (ops_to_replace_data + i));
  //   status = context->ReplaceNodeSubsetsWithDelegateKernels(
  //     context, kRegistration, ops_to_replace_part, delegate);
  //   TfLiteIntArrayFree(ops_to_replace_part);
...
TfLiteStatus opsToReplace(TfLiteContext* context, TfLiteDelegate* delegate, TfLiteIntArray *&ops_to_replace){
  ops_to_replace = GetOpsToReplace(context);
  if (ops_to_replace == nullptr) return kTfLiteError;
  return kTfLiteOk;
}
