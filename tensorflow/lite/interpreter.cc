...
TfLiteStatus Interpreter::Invoke(std::string fileName, bool profiling, std::vector<TfLiteDelegate *> delegates) {
  ...
  TF_LITE_ENSURE_STATUS_WITH_SCOPED_INSTRUMENTATION(
      scoped_runtime_event, primary_subgraph().Invoke(fileName, profiling, delegates));
