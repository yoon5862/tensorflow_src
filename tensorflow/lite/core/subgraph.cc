...
TfLiteStatus Subgraph::RecoredProfiling(std::string fileName, int op, size_t memory, double compute_time, const char* op_name){
  std::fstream file;
  file.open(fileName, std::ios::app);
  std::string op_name_str(op_name);

  std::string json_data = "{\"op\": \"" + std::to_string(op)+ "_" + op_name_str + "\", \"memory\": " + std::to_string(memory) + ", \"time\": " + std::to_string(compute_time) + "}";
  file<< json_data << "\n";
...
TfLiteStatus Subgraph::Invoke(std::string fileName, bool profiling, std::vector<TfLiteDelegate *> delegates) {
  // ModifyGraphWithDelegate(delegates[0]);
  ...
  // hack here!
  // put lazy delegate here!
  // inference time
  // supported operation candidate here!

  std::vector<std::set<int>> delegate_candidates;
  for(int d = 0; d < delegates.size(); d++){
    SwitchToDelegateContext();
    TfLiteIntArray *ops_to_replace = nullptr;
    status = delegates[d]->opt_node(&context_, delegates[d], ops_to_replace);
    SwitchToKernelContext();

    // std::cout << delegates[d]->delegateName << std::endl;

    if(status == kTfLiteError) return kTfLiteError;
    if (ops_to_replace == nullptr) {
      std::cerr << "ops_to_replace is nullptr!" << std::endl;
      return kTfLiteError;
    }
    int ops_to_replace_size = ops_to_replace->size;
    int *ops_to_replace_data = ops_to_replace->data;
    std::set<int> delegate_candidate;

    for(int op = 0; op < ops_to_replace_size; op++) delegate_candidate.insert(*(ops_to_replace_data + op));
    delegate_candidates.push_back(delegate_candidate);
  ...
  for (int execution_plan_index = 0;
    ...
    // put lazy delegate here!
    // first: split execution paln
    // second: put execution plan originaly

    // inference time to check Compute Core(CU) is busy.
    // If it is busy, offload to other CU
    // Note That we should plan custom memory arenao in Final target hardware.
    // In Respberry Pi 4, just use arenao plan algorithm is best.
    if(delegate_candidates[0].find(execution_plan_index) != delegate_candidates[0].end()){
      int execute_node = execution_plan_index;
      TfLiteIntArray *lazy_delegate_ops = ConvertArrayToTfLiteIntArray(1, &execute_node);

      SwitchToDelegateContext();
      status = delegates[0]->CustomPrepare(&context_, delegates[0], lazy_delegate_ops);
      SwitchToKernelContext();
      TfLiteIntArrayFree(lazy_delegate_ops);
    } else{
    ...
    if (execution_plan_index == next_execution_plan_index_to_prepare_) {
      TF_LITE_ENSURE_STATUS(PrepareOpsAndTensors()); // arenao plan
    ...
    auto elapsed_ms = std::chrono::duration<double, std::milli>(end - start);


    RecoredProfiling(fileName, execution_plan_index, 0, elapsed_ms.count(), op_name);
...
TfLiteStatus Subgraph::ModifyGraphWithDelegate(TfLiteDelegate* delegate) {
  ...
  std::cout << "before execution plan " << execution_plan_.size() << std::endl;
  ...
  std::cout << "after execution plan " << execution_plan_.size() << std::endl;
