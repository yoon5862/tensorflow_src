...
class Subgraph {
 public:
  ...
  // AllocateTensors().
  // Returns status of success or failure.
  TfLiteStatus Invoke(std::string fileName="", bool profiling=false, std::vector<TfLiteDelegate *> delegates={});

  TfLiteStatus RecoredProfiling(std::string fileName, int op, size_t memory, double compute_time, const char* op_name);
